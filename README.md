...
这是一个基于 Android 真机的本地自动化工具，提供命令行流程与 HalloTickets 状态看板。看板可显示设备状态、运行环境、任务进度和手机画面，并可在 Windows 上创建独立窗口的桌面图标。

> ✅ **适合你**：有一台安卓真机、愿意完成一次 Python / ADB 环境配置、想为自己或家人管理购票流程。
> ⛔ **不适合你**：没有安卓真机（iOS / 纯 PC / Mac 无真机都跑不了），或期待绕过平台验证与支付确认。这不是保证购票成功的服务。

## Windows 桌面版

Release 中的 `HalloTickets-Windows-Desktop-*.zip` 是完整的 Windows 启动包。解压后：

1. 安装 Python 3.10 ~ 3.13、Poetry 与 Android Platform Tools，并用 `poetry install` 安装项目依赖。
2. 双击根目录的 `Install-HalloTickets-Desktop.cmd`，桌面会生成 HalloTickets 图标。
3. 双击桌面图标，即可用 Edge 或 Chrome 的独立应用窗口打开本地看板。
...
| 基础操作 | 愿意完成一次安装并能看懂报错；日常可使用状态看板和桌面图标 |
...
> 公开版本只维护 `Mobile + 安卓真机` 路线；历史直连实现不再发布。
...
历史桌面直连原型不包含在公开发布物中，也不受支持。请使用上面的 [五分钟跑通 Mobile](#五分钟跑通-mobile) 和 HalloTickets 本地看板。
